...
local arrayFilter = Util.arrayFilter
...
function showWindow () LrFunctionContext.callWithContext ("", function (context)
    ...
    while true do
        ...
        prop.recentFiles = prefs.debugRecentFiles or {}
        ...
        local verb = LrDialogs.presentModalDialog {title = "Debug Script", 
            ...
            f:row {
                f:combo_box {value = bind ("filename"), width_in_chars = 45,
                    immediate = true, items = prop.recentFiles},
        ...
        if prop.filename ~= "" then 
            prop.recentFiles = arrayFilter (prop.recentFiles, function (f)
                return f ~= prop.filename end)
            table.insert (prop.recentFiles, 1, prop.filename)
            while #prop.recentFiles > 20 do table.remove (prop.recentFiles) end
            end
        prefs.debugRecentFiles = prop.recentFiles

        if verb == "options" then
        elseif prop.filename == "" then
